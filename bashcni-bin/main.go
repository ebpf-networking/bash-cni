...
func init() {
    var logpath = "/var/log/bash-cni-plugin.log"
...
func main() {
    ...
    case "ADD":
        ...
        cmdStr = fmt.Sprintf("echo \"{\\\"cniVersion\\\": \\\"0.3.1\\\", \\\"type\\\": \\\"bash-cni\\\", \\\"name\\\": \\\"%s\\\", \\\"ipam\\\": { \\\"type\\\": \\\"host-local\\\", \\\"subnet\\\": \\\"%s\\\" }}\" | CNI_COMMAND=ADD CNI_CONTAINERID=%s CNI_NETNS=%s CNI_IFNAME=%s CNI_PATH=%s /opt/cni/bin/bridge", myname, subnet, cni_containerid, cni_netns, cni_ifname, cni_path)
        args = []string{"-c", cmdStr}
        output, err = runcmd("sh", args, true)
        if err != nil {
            Log.Println("Error:", output)
